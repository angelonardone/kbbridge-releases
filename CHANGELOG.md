...
> **MINOR bump** — ten plugin versions pulled (v1.10.3 → v1.11.6). Headline: **`WorkPanel` is
> a first-class object type and its Win Form has a full visual editor** — render, select, move,
> resize, align, multi-select, add/remove grid columns and tab pages, and edit every property
> from the GX Properties panel, with the `.gxWinForm` XML preserved byte for byte through the
> round-trip. The desktop-form object that the modern GeneXus IDE no longer lets you create is
> now editable in KBEditor. Also in the range: a **data-loss fix** where editing an SDT member's
> property could corrupt an unrelated source file, and the watcher fixes that stopped saves from
> being silently swallowed after a KB re-export.
>
> ⚠️ **Requires KB Sync Engine v1.7.6+** (v1.7.6.1+ for the full Work Panel property set) — a
> larger jump than usual. Older Engines will not round-trip `.gxWinForm` files.
>
> 📎 **Upstream renumbered its own versions.** What we shipped in KBEditor 1.9.0 as
> *Plugins v1.9.17 / v1.9.18 / v1.9.19* is now called **v1.10.0 / v1.10.1 / v1.10.2** in the
> plugins repo — same commits, same content, renamed in place because the `File` object type
> warranted a MINOR bump on their side. The 1.9.0 entry below is left as it was published; no
> KBEditor release is missing.

### Plugins

#### v1.11.6 — 2026-08-10
- Added: [sync-status] **Builds canceled because the KB needs a database reorganization are labeled "Reorg pending"** instead of the generic "Canceled", with the Engine's explanation of what to reorganize expanded by default. The existing Retry button re-sends the same request once the reorg has been applied from the GeneXus IDE (Build > Impact Database Tables). Recognised by the request-level message code, so cancellations for any other reason still read as plain Canceled.

#### v1.11.5 — 2026-08-07
- Changed: **A Win Form is now a `.gxWinForm` file, and `.gxForm` means a web form and nothing else** — the two are different XML formats that had been sharing one extension, and that collision is what kept a Transaction's desktop form from being synchronised at all. A Work Panel's form is now `<Name>.WorkPanel.gxWinForm`, and a Transaction can carry three files (source, web form, Win Form). File contents are unchanged and there is nothing to migrate — Work Panel support was never released, so no file in the field carries the old name. **Requires KB Sync Engine v1.7.6+**.
- Changed: [visual-editor] **The Win Form editor opens by extension rather than by object type**, so it covers a Transaction's Win Form with no further work. Everything that derives one companion file from another was updated with it, and the "is this variable/attribute used on the form?" check now looks at *both* forms.
- Changed: [core] `.gxWinForm` has its own file icon, language association and syntax; the **Open Win Form** link above a source points at it, and the back-to-source button appears in the Win Form toolbar. Where an object has both forms, the link above the source still offers only the web one.
- Changed: [sync-status] `.gxWinForm` files are watched, tracked and grouped under their object like the other companion files — without this, editing a Win Form produced no pending change at all.
- Added: [core-lib] **Work Panel property vocabulary expanded from 8 to 38 entries** in the shared properties config, so the GX Properties panel exposes the object's full set (`Triggers`, `ConfirmTransactions`, `BorderStyle`, `ModalDialog`, `JoinManagement`, …). All labels reuse tokens already coined for Transaction / Procedure / SDPanel. **Requires KB Sync Engine v1.7.6.1+**.

#### v1.11.4 — 2026-08-06
- Fixed: [visual-editor] **Win Form `Position` (Left/Top/Width/Height) now nests under `Layout`** in the GX Properties panel, matching the designer — the 19 schemas that declare them moved to the hierarchical `category: "Layout/Position"` convention.
- Fixed: [visual-editor] **A Dynamic Combo/List Box bound to a short attribute but showing a long description is sized by the description**, in both Win Form and Web Form — a `TestId` (Numeric(4)) drawn as a combo whose description comes from `TestTitle` (Character(50)) was ~4 characters wide instead of ~50. Sizing now resolves from the first attribute of `Item Descriptions` (or `Item Values`), falling back to the bound attribute.
- Fixed: [visual-editor] **The same sizing now also works with `Data Source From = Data Provider`** — the editor walks `.gxForm` → DP `.gxSource` → output SDT → field, reading the declared length off the field's `DataType`. Any broken step falls back silently to the previous behaviour.
- Fixed: [visual-editor] **A Win Form Radio Button draws one radio per option** instead of a single 13-pixel circle labelled with the attribute name — `Values = Name1:1,Name2:2` renders one glyph per pair, vertical or horizontal per `Radio Direction`, and auto-resizes accordingly.
- Fixed: [visual-editor] **`Data Provider` and `Parameters` in a Win Form get their picker / code editor**, matching the Web Form — they were declared as plain read-only strings.
- Added: [visual-editor] **`Item Values` / `Item Descriptions` become dropdowns of the DP output SDT's fields** when `Data Source From = Data Provider`, populated by walking the DP → SDT chain. `Item Values` filters to fields type-compatible with the bound attribute, both preserve the SDT's declaration order, and the first compatible field is pre-selected as a hint — written to the `.gxForm` only when the user explicitly picks it.
- Fixed: [visual-editor] **The `…` picker of any `type: reference` property now works from the Win Form editor** — only the Web Form and Pattern Editor were wiring that bridge, so the button silently did nothing.

#### v1.11.3 — 2026-08-05
- Fixed: [visual-editor] **A Win Form control bound to an attribute keeps its Control Info, and its control type survives an edit** — where those settings live is the opposite of the variable case (form file vs. source), and only the variable case was handled.
- Added: [visual-editor] **A Web Panel attribute now shows the full Control Info for its control type** — a `Dynamic Combo Box` used to display only `Add Empty Item`; 13 properties were added (`Data Source From`, `Data Provider`, `Parameters`, `Item Values`, `Item Descriptions`, `Sort Descriptions`, `Conditions`, `Instantiated Attributes`, `Filter Operator`, `Suggest Max Items`, `Case Sensitive`, `Radio Direction`, `Control Title`), each visible only when its control type applies.
- Fixed: [visual-editor] **Edit-only properties no longer appear on other control types** in a Web Panel — `Input Type`, `Suggest`, `Auto correction`, `Auto capitalization`, `Notify Context Change` and `Is Password` are now hidden when `Control Type ≠ Edit`.
- Added: [visual-editor] **The shared attribute picker gained an "attributes only" mode** — `Item Values`, `Item Descriptions` and `Instantiated Attributes` only accept attributes, so offering variables there was misleading.
- Fixed: [visual-editor] **The code editor popup (`…`) of `Conditions` / `Parameters` works outside the Pattern Editor** — it was routed exclusively to the Pattern Editor, so clicking `…` on a Web Panel attribute did nothing. Both routes now use the shared native code-editor service (temp file in a split below, `Ctrl+S` applies and closes).
- Fixed: [visual-editor] **The GX Properties panel stays populated while that popup is open, and repeated open/save cycles never lose the change** — three mechanisms cover the three failure modes seen in testing (context snapshot + repopulate, serialised open/close so two cycles cannot race, and a fallback to the last non-empty context).
- Fixed: [visual-editor] **Win Form `Conditions` / `Instantiated Attributes` get their `…` button**, and `Item Values` / `Item Descriptions` / `Instantiated Attributes` open the attribute-only picker.
- Fixed: [visual-editor] **`Data Source From` for a Win Form dynamic combo/list no longer defaults to `Values`** — a value the designer never offers; it now drops that option and defaults to `Attributes`.
- Fixed: [visual-editor] **`Values` on a Win Form combo / list box / radio button gets the two-column values editor** instead of an empty read-only text box.
- Fixed: [visual-editor] **Seven Control Info properties whose stored XML name differs from their label now read and write the right entry** — `Item Values`, `Item Descriptions`, `Suggest Max Items`, `Sort Descriptions`, `Radio Direction`, `Conditions` and `Instantiated Attributes` are stored as `ControlItemValues`, `ControlItemDescription`, `MaxItems`, `ControlSortDescription`, `ControlDirection`, `ControlWhere`, `ControlRestrictedBy`. Values already saved in a form are now visible and editable.

#### v1.11.2 — 2026-08-04
- Fixed: [visual-editor] **A Win Form combo, list box, radio button or check box shows its actual values instead of blank fields** — `Values`, `Item Values`, `Suggest`, `Checked Value`, `Data Provider` and the rest of the *Control Info* block are **not stored in the `.gxForm` at all**: GeneXus keeps them on the bound variable, inside `#Variables` of the `.WorkPanel.gxSource`. The editor now reads both sources and shows them merged, as the designer does.
- Fixed: [visual-editor] **Editing one of those properties no longer loses the change silently** — the fields were shown empty *and editable*, and whatever was typed went into the `.gxForm`, which the Sync Engine does not read on import. The value reappeared in the panel (re-read from the same wrong place), so the edit looked saved and never reached GeneXus. Each property now goes to the file that owns it.
- Added: [visual-editor] **Complex properties in GX Properties are editable at last**, each with the right editor — `Values` opens the two-column Values Editor, `Item Values` / `Item Descriptions` / `Instantiated Attributes` the attribute picker, `Conditions` / `Data Provider Parameters` the code editor, `Data Provider` a KB picker, `Font` the font editor. They used to be greyed out and read-only on *every* panel, so this reaches well beyond Win Forms.
- Fixed: [visual-editor] **Changing a data control's `Control Type` keeps both files in step** — the type is carried by the XML tag (`<CTRL_COMBO>`) while the variable records it as a property; only the tag was being rewritten. **Requires KB Sync Engine v1.5.22+**.
- Fixed: [visual-editor] **Control Info properties of a dynamic combo/list appear and disappear as in the designer** — the SDK states every dependency (`Data Provider` only when the source is *Data Provider*, `Item Values` only when *Attributes*, the `Suggest` family only when suggestions are on) and all of them were being listed at once.
- Fixed: [visual-editor] **53 properties whose default GeneXus computes at runtime no longer show a wrong one** — where the definition file and the designer disagree, the property falls back to the value declared at the variable level; where they are the same value spelled differently (`Begins with` vs `BeginsWith`) they are reconciled instead of dropped.
- Fixed: [visual-editor] **Selecting a Work Panel variable in the `.gxSource` shows its Control Info** — the panel looked for the binding as an XML attribute (web form / abstract layout), but a Win Form writes it as an element, so no Work Panel variable was ever recognised as being on its form.
- Fixed: [visual-editor] **A variable lists only the Control Info its control type actually has** — a combo used to show the properties of a plain edit and of a dynamic combo alongside its own, twenty at once.
- Fixed: [visual-editor] **`Auto capitalization` offers the options GeneXus accepts** (*None*, *First word*, *First letter of each word*) instead of the mobile-platform set, and `Auto correction` defaults to *True* as in the IDE. Both also show on the form, not only in the source.
- Added: [visual-editor] **Changing a variable's control type removes the settings that no longer apply**, listing what was dropped in the *GeneXus Visual Editor* output channel; undo restores it in one step.
- Fixed: [visual-editor] **Editing a variable's properties no longer mixes LF into a CRLF `.gxSource`** — every edit used to leave a few lines in a different style and show up as diff noise.
- Fixed: [analyzer] **`DataProvider` is accepted as a property name inside a variable's property block** — `DataProvider = 'RetMenus'` next to `ControlType = 'DynamicCombo'` (the standard way to bind a dynamic combo to a DP) failed with `Mismatched input 'DataProvider'` because it is a dedicated object-type keyword.

#### v1.11.1 — 2026-08-03
- Added: [visual-editor] **The Toolbox offers Win Form controls and dropping one on a Work Panel form creates it** — Button, Text, Attribute/Variable, Rectangle, Line, Image, Grid, Tab, Group Box. The drop lands in whatever container is under the cursor however deep (a control dropped on a tab page belongs to that page; one dropped on a grid becomes a column), and inherits the font, colours and border style of its new siblings rather than the SDK defaults.
- Added: [visual-editor] **Controls can also be created from the canvas right-click menu**, inserted exactly where the menu was opened.
- Added: [visual-editor] **Picking up a Toolbox item puts the editor in "place" mode** as a fallback — VSCode renders each panel in its own iframe and Chromium withholds drag events from an iframe that has not been focused recently, so a drag intermittently delivers nothing. A bar then reads *"Click on the form to place the … · Esc to cancel"* and a single click drops the control.
- Fixed: [visual-editor] **Buttons show their real caption instead of the word "Confirm"** — the format has no `Caption` property: button, label, check box, radio button, group box and tab page captions live in `CaptionExpression` as a token stream. All six types now read and write it, and refuse to overwrite a computed expression.
- Fixed: [visual-editor] **Controls that size themselves are drawn at their real size** — while `Auto Resize` is on (the default) GeneXus *ignores* the stored `Width`/`Height` and recomputes from the data type and font. Resizing with the mouse now writes `Auto Resize = False` alongside the new size, as the designer does.
- Added: [visual-editor] **Same Width / Same Height / Same Size on a multi-selection** (`W` `E` `S`), in both the Win Form and Procedure Layout editors.
- Added: [visual-editor] **`Auto Resize` is editable from the panel** on every control that has it.
- Added: [visual-editor] **Win Form control properties now come from the Sync Engine's catalogue instead of a hand-made list** — the schemas had been reconstructed from an exported `.gxForm`, and since the Engine only externalises what differs from the default, every property left at its default was missing: a button showed 11 properties and actually has 22, a data control went from 20 to 64. The catalogue also carries each property's real name *inside the file*, so the editor no longer writes properties GeneXus silently ignores. **Requires KB Sync Engine v1.5.22+**.
- Added: [visual-editor] **Properties are listed in the designer's order, under its headings, with its conditional behaviour** — IDE-only properties (`Tag`, `Enabled`, the individual font flags) are gone from the grid, `Width`/`Height` are greyed out while `Auto Resize` is on, `Horizontal Alignment` disappears on a check box.
- Fixed: [visual-editor] **A Win Form combo, check box or dynamic combo is shown as what it is** — the kind is carried by the XML tag alone (`<CTRL_COMBO>`, `<CTRL_CHECK>`), with no property saying so, and the editor was reading a `Control Type` property the file never has, so everything rendered as a plain edit.
- Fixed: [sync-status] **Objects the user never touched no longer appear as pending changes after a window focus** — a fix from two days earlier (preserving the snapshot entry when the export filter skipped a change) injected false positives in the opposite direction: once the Engine re-exported *any* other object, the shared exports file's mtime moved and the protection window quietly stopped applying, so the leftover diff was reported as Modified. The reconciler now absorbs the actual state of disk on every pass.

#### v1.11.0 — 2026-08-02
- Added: **`WorkPanel` (Win Form) is a first-class GeneXus object type across every extension** — a `.WorkPanel.gxSource` is parsed, validated, indexed, iconified and searchable exactly like a Web Panel. Work Panels are the legacy desktop-form object that GeneXus still supports but the modern IDE no longer lets you create. Before this release the analyzer flagged `WorkPanel` as an unknown keyword on line 1, the object never entered the KB cache, and the Explorer showed the generic fallback icon. **Requires KB Sync Engine v1.5.19+**.
- Added: [analyzer] The `WorkPanel` header accepts the same sections as a Web Panel (`#Rules`, `#Events`, `#Conditions`, `#Variables`), and **`&WorkPanel` keeps working as a variable name** — the same regression that hit `&File` was headed off up front this time, in both the variable and member positions.
- Added: [visual-editor] **New visual editor for the Win Form — it renders as a form, not as raw XML** — the frame surface, rectangles and lines, labels, 3D-sunken data fields, raised buttons, the grid with its column titles, and the tab control with clickable pages. Containers nest to any depth and coordinates follow the IDE's rules (children of a tab page are relative to the page; grid columns have no coordinates — their order in the document is their order on screen). Zoom with Ctrl + wheel, drag to pan, `Fit` to frame.
- Added: [visual-editor] **Win Forms are editable — properties and the layout itself.** Setting a property back to its default **removes it from the file** rather than writing the default, exactly as the Engine stores forms; new properties are inserted where the Engine would put them, and existing ones are never reordered, so a form written by GeneXus keeps its shape and edits produce a minimal diff.
- Added: [visual-editor] **The `.gxWinForm` XML survives an open/close cycle byte for byte** — the parser and serializer preserve the `encoding="utf-16"` declaration the Engine writes (even though the file is UTF-8), CRLF endings, two-space indentation, the four-line shape of every `<Property>`, empty blocks, the absence of a trailing newline, and the original property order. A verification script (`scripts/test-winform-roundtrip.js`) checks this against any folder of forms.
- Added: [visual-editor] **Drag to move, eight handles to resize, alignment guides that snap** (Ctrl to place freely), arrow-key nudge (Shift for ten pixels), and a line — being zero pixels tall — offering only the two handles that can stretch it.
- Added: [visual-editor] **Multi-selection with Shift+click or a selection box**, dragged as a group in a single undo step, with a **primary** control (solid blue, the rest dimmed) anchoring the alignment commands.
- Added: [visual-editor] **Floating alignment toolbar** with the same commands, icons and shortcuts as the Procedure Layout editor: align (`L` `C` `R` `T` `M` `B`), distribute (`H` `V`), bring to front / send to back (`]` `[`).
- Added: [visual-editor] **Grid columns and tab pages can be added, removed and reordered** — *Arrange Columns* lists every column with its width and buttons to add, delete and move; adding a column asks for its binding through the shared picker and creates it the way the Engine writes them (no coordinates, inheriting font and colours from its siblings).
- Added: [visual-editor] **A data control's binding can be changed with the shared attribute/variable picker**, and **binding to an undeclared variable declares it in `#Variables`** — writing to the sibling `.WorkPanel.gxSource` with that section's existing indentation, producing its own entry in the changes file. GeneXus' predefined variables (`&Today`, `&Time`, …) are never declared.
- Added: [visual-editor] **Data fields without an explicit `Width` are sized from what they are bound to**, following `AttID → variable or attribute → domain → base type`, because the Engine only externalises properties that differ from the SDK default.
- Added: [visual-editor] **Controls extending past the form's edge are drawn overflowing**, with the same red "!" corner marker the Procedure Layout editor uses, and the canvas scrolls far enough to reach them.
- Added: [visual-editor] **Forms declared for more than one generator show a tab per variant** (`Windows` / `Text` / `Mobile`) — `Windows` is rendered, the others preserved untouched. A form marked `dynamic="true"` is **read-only with a banner offering to unlock it**, since the Engine regenerates those from the KB and would discard the changes.
- Added: [visual-editor] **`Work Panel` is in the "New Object" dialog** with its own icon; unlike other form-bearing types whose form is created empty, the Win Form is seeded with a minimal valid document, because an empty file is not a parseable Win Form.
- Added: [core] **New `WK` badge and dedicated icons for `.WorkPanel.gxSource` / `.gxProperties` / `.gxWinForm`**, plus `WorkPanel` header highlighting on line 1. The CodeLens above the source reads **"Open Win Form"**.
- Added: [sync-status] Work Panel changes show up with a `WkP` type badge — source and form already travelled to the Engine sharing one `qualifiedName`, like a Web Panel's pair.
- Fixed: [visual-editor] **A Win Form no longer opens in the web Form Editor, where it rendered as a stray text node and could be corrupted on save** — the custom editor claimed every `**/*.gxForm`, so the first save would have rewritten the document with the HTML serializer, destroying the `<Property>` structure the round-trip depends on.
- Fixed: [visual-editor] **The GX Properties panel populates from line 1 of a `Panel`, `WebComponent` and `WebMasterPage` source** — those keywords were missing from the object-declaration matcher.
- Fixed: [visual-editor] **"Save Object As…" rewrites the first line when duplicating a `WebMasterPage`** — the duplicate kept the original object name on its header, which the Engine would import as an edit of the original. `Table` and `Group` were missing too.
- Fixed: [analyzer] **`WebComponent` and `WebMasterPage` are no longer analysed as if they were Procedures** — both detection paths fell through to their `default` branch, so those sources were validated with Procedure rules.

#### v1.10.5 — 2026-08-02
- Fixed: [visual-editor] **Property edits on an SDT member no longer risk corrupting an unrelated source file** — a **data-loss bug** confirmed by the KB Sync Engine team: the writer searched the target file for the member's name and injected the property block on the first match, so if the target was actually another `.gxSource` declaring a variable with the same name (a `RefreshToken` variable in a Procedure while `SDTAuthResponse.RefreshToken` was being edited), it silently added the member's properties inside that variable's block. Two guards now block it — the writer refuses to run unless the file starts with an `SDT` / `StructuredDataType` declaration, and the change handler refuses any SDT member edit whose target is not an SDT source. A refusal surfaces a warning and a diagnostic block in the output channel; nothing is written to disk.
- Added: [sync-status] **The Builds tab shows whether the Engine auto-builds after every import, and with what scope** — so when a save arrives and no build appears you can tell whether auto-build is off (expected) or something failed. Older Engines get *"Info not available (Sync Engine < v1.5.20)"* rather than a misleading "off". **Requires KB Sync Engine v1.5.20+**.
- Fixed: [sync-status] **The same object no longer floods My Sources with 10+ near-identical "Skipped" cards after a re-export** — the dedup window widened from 5 seconds to 5 minutes per `(file, reason)`, and consecutive cards sharing a file and reason render as one card with a `×N` badge.
- Fixed: [sync-status] **Opening a project no longer produces dozens of "Skipped" cards for untouched objects** — a housekeeping filter that discards files older than the last bulk export was surfacing as user-visible skips. It still runs and is still logged, it just no longer clutters the panel.
- Changed: [sync-status] **Discarded save attempts are now opt-in, off by default** — a new **"Show discarded save attempts"** toggle on the Sync Settings tab. The buffer is populated in the background either way, so turning it on shows the last ~100 immediately.
- Fixed: [visual-editor] **Clicking an auto-sized control no longer marks the file as modified** — any gesture crossing the 3-pixel drag threshold was treated as a resize and unconditionally added `AutoResize='False'`, rewriting the file with a new mtime. `AutoResize` is now only turned off when the width or height actually changed.

#### v1.10.4 — 2026-08-01
- Fixed: [sync-status] **A re-exported object is observable by the watcher again** — the previous release fixed the 5-minute window but the object still went permanently deaf for two other reasons: the "clean up My Sources after a resolved re-export" step deleted *every* change entry of the file rather than only those predating the re-export, and the reconciler absorbed the new mtime into its snapshot even when the change had been skipped. Both are now aware of the re-export's completion timestamp.
- Added: [sync-status] **Saves the editor intentionally discards now show up with a "Skipped" badge and a human explanation** (echo of a KBBridge re-export, unresolved git merge markers, duplicate within 5 s, unrecognised GeneXus file), instead of being dropped silently and leaving the user with "I saved and nothing happened". Informational only — they never travel to the Engine and clear on reload.

#### v1.10.3 — 2026-07-31
- Fixed: [analyzer] **`&Header` and `&Footer` are accepted as variable names** — both are keywords because print-output Procedures use `Header … End` / `Footer … End` blocks, and they were shadowing the identifier when preceded by `&`. Method chains on them (`&Header.ToUpper()`) also parse.
- Fixed: [analyzer] **Negative numbers in a Domain's `EnumValues` no longer break parsing with a spurious `Missing 'EndIf'` further down the file** — an enumerated domain using negative error codes (`Value: -32700`) sent the parser into error recovery that drifted until it found a `]`. Negative numbers are now accepted anywhere a simple value is expected.
- Fixed: [sync-status] **A save on an object KBBridge had re-exported earlier shows up in My Sources instead of being swallowed** — the export filter kept a 5-minute window open per object, and the "last export timestamp" was computed against the shared exports file's mtime, which any *other* object's export bumped, renewing the filter indefinitely. It now skips a change only when the file's mtime is within a few seconds of the export write itself.

### Platform
- Bundle rebuilt from plugins v1.11.6 with `build-bundle.py --compile`: **759 JS / 10 CSS / 59 JSON, 2.4 MB** (was 750 / 9 / 39 / 2.3 MB). Per extension: analyzer 248, **visual-editor 217 (was 187)**, language-server 186, sync-status 66, validator 43, controller 41, core 39.
- No packaging changes were needed for the Win Form editor's assets: `build-bundle.py` already copies the visual editor's `schemas/` and `webview/` directories wholesale, so the **20 new `schemas/winform-controls/*.json`** control schemas (+20 JSON), the **new `webview/winform-editor/styles.css`** (+1 CSS) and the compiled `winform-editor.js` were picked up automatically. No new runtime dependency was introduced.
- Shared configs synced (`core-lib/shared/*.json` → `kbbridge/shared/`): `gx-properties-config.json` grew 256 KB → 280 KB (Work Panel's property vocabulary 8 → 38 entries). The `controlTypes` catalogue the Win Form editor consumes is generated KB-side by the Sync Engine and checked into the plugin repo as the `schemas/winform-controls/` files, so it needs no mirror here.
- VSIX skeletons: `genexus-language-core`, `genexus-sync-status` and `genexus-visual-editor` package.json synced by `sync-vsix-skeletons.py` — the `genexus.winFormEditor` custom editor for `**/*.gxWinForm`, the `kbbridge.unlockWinForm` command, `.WorkPanel.gxSource`/`.gxProperties` and `.gxWinForm` associations, and the `gxSync.showSkippedAttempts` setting.
- Manual VSIX declaratives (not covered by skeleton sync, which only rewrites `package.json`): updated `genexus.tmLanguage.json` in the language-core VSIX (`WorkPanel` added to the line-1 object-declaration match) and `icon-theme.json` + `workpanel.svg` / `workpanel-form.svg` in the icons VSIX. Also added `procedure-layout.svg`, which `icon-theme.json` already referenced but which had drifted out of the VSIX in an earlier release — a dead icon reference until now.
...
### Bundle
- **Must be uploaded to `keygenapi.kbbridge.com`** — the visual editor gained 30 files (the whole Win Form editor: provider, model, XML parser/serializer, type resolver, variable binding, DP field resolver, 20 control schemas, webview + stylesheet), and analyzer, validator, core, core-lib and sync-status all changed as well.
